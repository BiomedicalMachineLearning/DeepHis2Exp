...
class mixer_transformer(nn.Module):
    def __init__(self,channel=32, kernel_size=5, dim=1024,
                 depth1=2, depth2=8, 
                 heads=8, dim_head=64, mlp_dim=1024, dropout = 0.2,
                 gnn="GIN", co_conv=False, residual=False, 
                 avgpool=False, maxpool=False, channelattn=False, 
                 lstm=True,
                ):
        super().__init__()
        ...
        self.layer3=GNN(num_feature=1024, nhid=1024, residual=residual, gnn=gnn,
                avgpool=avgpool, maxpool=maxpool, channelattn=channelattn, 
                lstm=lstm, dropout=dropout,)
        if co_conv:
            self.layer4=GIN_HSG(num_feature=dim, nhid=dim, residual=residual, gnn=gnn,
                avgpool=avgpool, maxpool=maxpool, channel=channel, lstm=lstm, dropout=dropout,)
...
class ViT(nn.Module):
    def __init__(self, channel=32,kernel_size=5,dim=1024, 
        ...
        self.transformer = mixer_transformer(
            ...
            gnn, co_conv, residual, avgpool, maxpool, channelattn, lstm,)
...
class Hist2ST_GNN(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, fig_size=112, label=None, 
                 ...
                 zinb=0, nb=False, bake=0, lamb=0, gnn="GIN", co_conv=False, 
                 residual=False, avgpool=False, maxpool=False, channelattn=False, 
                 lstm=True,
                ):
        super().__init__()
        ...
        self.vit = ViT(
            ...
            co_conv=co_conv, residual=residual,
            avgpool=avgpool, maxpool=maxpool, channelattn=channelattn, lstm=lstm,
        )
