...
class GNN(nn.Module):
    def __init__(self, gnn="gcn", in_channels=512, out_channels=512,):
        super().__init__()
        self.gnn = gnn
        if self.gnn == "gcn":
            self.conv = nn.ModuleList([GCNConv(in_channels, out_channels) for i in range(3)])
        elif self.gnn == "gat":
            self.conv = nn.ModuleList([GATConv(in_channels, out_channels) for i in range(3)])
        elif self.gnn == "gin":
            self.conv = nn.ModuleList([GINConv(nn.Sequential(nn.Linear(in_channels, out_channels), nn.ReLU(), nn.Linear(out_channels, out_channels))) for i in range(3)])
        self.lstm = nn.LSTM(out_channels, out_channels, 2)

    def forward(self, x, edge_index, edge_weights):
        if self.gnn == "gin":
            jk = [gcn(x, edge_index).unsqueeze(0) for gcn in self.conv]
        else:
...
class DeepPT_messagepass(pl.LightningModule):
    def __init__(self, gnn="gcn", n_genes=1630, hidden_dim=512, learning_rate=1e-4,):
        super().__init__()
        ...
        self.GNN = GNN(gnn=gnn, in_channels=hidden_dim, out_channels=hidden_dim)
...
class CNN_GNN_AE(DeepPT_messagepass):
