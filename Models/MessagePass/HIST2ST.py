...
class Hist2ST(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, label=None, 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        R = torch.Tensor([pearsonr(pred[g],exp[g])[0] for g in range(self.n_genes)]).mean()
        self.log('PCC', R, on_epoch=True, prog_bar=True, logger=True)
        return loss
