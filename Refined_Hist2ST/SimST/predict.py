...
def pk_load(fold,mode='train', pretrained_model="efficientnet", flatten=False,dataset='her2st',r=4,ori=True,adj=True,prune='Grid',neighs=4):
    assert dataset in ['her2st','cscc']
    if dataset=='her2st':
        dataset = ViT_HER2ST(
            train=(mode=='train'),fold=fold, pretrained_model=pretrained_model, flatten=flatten,
    ...
    return dataset
def test(model,test,Regclass=False,device='cuda'):
    ...
    loss=0
    with torch.no_grad():
        for patch, position, exp, label, adj, *_, center in tqdm(test):
            patch, position, adj, label = patch.to(device), position.to(device), adj.to(device).squeeze(0), label.to(device)
            pred = model(patch, position, adj)[0]
            if Regclass:
                cell_type_prob = model(patch, position, adj)[3]
#                 cell_type_prob = cell_type_prob.cpu().numpy()
    ...
    if Regclass:
        return adata,adata_gt,cell_type,accuracy
    else:
