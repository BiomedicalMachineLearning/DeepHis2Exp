...
gene_list = list(read_gene_set("../data/pfizer/"))
...
test_sample = "VLP82_A" ########## TODO FIX THIS ONCE ADATA_ALL IS COMPLETE
...
########## TODO FIX THIS ONCE ADATA_ALL IS COMPLETE
# adata_all_train_valid = adata_all 
...
df.to_csv("../results/pf/stnet_cor_{}.csv".format(test_sample))


with open("../results/pf/stnet_times.txt", 'a') as f:
    f.write(f"{test_sample} {end_train - start_train} - {time.strftime('%H:%M:%S', time.localtime())}")

with open(f"../results/pf/stnet_preds_{test_sample}.pkl", 'wb') as f:
