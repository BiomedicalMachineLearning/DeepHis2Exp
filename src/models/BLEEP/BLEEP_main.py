...
parser.add_argument('--data_name', type=str, default='visium_bc', help='')
parser.add_argument('--fold', type=int, default=None, help='')
parser.add_argument('--dim', type=int, default=None, help='spot_embedding dimension (# HVGs)')
parser.add_argument('--holdout', type=int, default=None, help='Set to 1 to holdout 10X data')


def build_loaders(args):
    # slice 3 randomly chosen to be test and will be left out during training
    print(f"Building loaders for {args.data_name} dataset")
    if args.data_name == "liver":
        ...
        datasets = [dataset, dataset2, dataset4]
    elif args.data_name == "visium_bc":
        samps1 = ["1142243F", "CID4290", "CID4465", "CID44971", "CID4535", "1160920F"]
        # samps2 = ["block1", "block2", "FFPE", "1168993F"]
        samps2 = ["block1", "block2", "FFPE"]
        if args.holdout:
            samps2 = []
            samps1 = ["1142243F", "CID4465", "CID44971", "CID4535"]

        datasets = [
            CLIPDataset(image_path = f"data/Alex_NatGen/{samp}/image.tif",
                   spatial_pos_path = f"data/Alex_NatGen/{samp}/spatial/tissue_positions_list.csv",
                   # reduced_mtx_path = f"data/filtered_expression_matrices/{samp}/harmony_matrix.npy",
                   reduced_mtx_path =  f"data/filtered_expression_matrices/{samp}/hvg_matrix_plusmarkers.npy",
                   barcode_path = f"data/Alex_NatGen/{samp}/filtered_count_matrix/barcodes.tsv.gz")
            for samp in samps1
        ] + [
            CLIPDataset(image_path = f"data/Breast_Cancer_10x/{samp}/image.tif",
                   spatial_pos_path = f"data/Breast_Cancer_10x/{samp}/spatial/tissue_positions_list.csv",
                   # reduced_mtx_path = f"data/filtered_expression_matrices/{samp}/harmony_matrix.npy",
                   reduced_mtx_path =  f"data/filtered_expression_matrices/{samp}/hvg_matrix_plusmarkers.npy",
                   barcode_path = f"data/Breast_Cancer_10x/{samp}/filtered_feature_bc_matrix/barcodes.tsv.gz")
            for samp in samps2
        ]
        if isinstance(args.fold, int): # test fold
            datasets.pop(args.fold)
            print("Test fold: ", args.fold)
    elif args.data_name == "visium_bc_6":
        samps1 = ["1142243F", "CID4290", "CID4465", "CID44971", "CID4535", "1160920F"]
        # samps2 = ["block1", "block2", "FFPE", "1168993F"]
        samps2 = ["block1", "block2", "FFPE"]
        ...
        datasets = [
            CLIPDataset(image_path = f"data/Alex_NatGen/{samp}/image.tif",
                   spatial_pos_path = f"data/Alex_NatGen/{samp}/spatial/tissue_positions_list.csv",
                   # reduced_mtx_path = f"data/filtered_expression_matrices/{samp}/harmony_matrix.npy",
                   reduced_mtx_path =  f"data/filtered_expression_matrices/{samp}/hvg_matrix_plus6.npy",
                   barcode_path = f"data/Alex_NatGen/{samp}/filtered_count_matrix/barcodes.tsv.gz")
            for samp in samps1
        ] + [
            CLIPDataset(image_path = f"data/Breast_Cancer_10x/{samp}/image.tif",
    ...
    elif args.data_name == "visium_bc_6subset":
        samps1 = ["1142243F", "CID4290", "CID4465", "CID44971", "CID4535", "1160920F"]
        # samps2 = ["block1", "block2", "FFPE", "1168993F"]
        samps2 = ["block1", "block2", "FFPE"]
        ...
        datasets = [
            CLIPDataset(image_path = f"data/Alex_NatGen/{samp}/image.tif",
                   spatial_pos_path = f"data/Alex_NatGen/{samp}/spatial/tissue_positions_list.csv",
                   # reduced_mtx_path = f"data/filtered_expression_matrices/{samp}/harmony_matrix.npy",
                   reduced_mtx_path =  f"data/filtered_expression_matrices/{samp}/hvg_matrix_plus6_subset.npy",
                   barcode_path = f"data/Alex_NatGen/{samp}/filtered_count_matrix/barcodes.tsv.gz")
            for samp in samps1
        ] + [
            CLIPDataset(image_path = f"data/Breast_Cancer_10x/{samp}/image.tif",
        ...
        datasets = [
            HERDataset(image_path = f"data/her2st/images/{samp}.jpeg",
               ...
               # reduced_mtx_path = f"data/filtered_expression_matrices/{samp}/harmony_matrix.npy",
               reduced_mtx_path =  f"data/filtered_expression_matrices/her2/{samp}/hvg_matrix_plusmarkers.npy",
    ...
    elif args.data_name == "her2_subset":
        names = os.listdir("data/her2st/ST-cnts")
        names.sort()
        names = [i[:2] for i in names]
        # print(names)
    ...
    print("Train samples:", len(datasets))
...
def main():
    ...
    # HACK to fix port sharing
    if not args.fold:
        args.fold =1
    init_method = args.init_method[:-1] + str(args.fold)
    args.fold = None
    dist.init_process_group(backend=args.dist_backend, init_method=init_method, world_size=args.world_size, rank=rank)
    ...
    if args.model == "clip":
        ...
        model = CLIPModel_ViT_L(spot_embedding=args.dim).cuda(current_device)
        print("Image encoder is ViT_L")
    elif args.model == "resnet101":
        model = CLIPModel_resnet101(spot_embedding=args.dim).cuda(current_device)
        print("Image encoder is ResNet101")
    elif args.model == "resnet152":
        model = CLIPModel_resnet152(spot_embedding=args.dim).cuda(current_device)
        print("Image encoder is ResNet152")
    else:
        model = CLIPModel(spot_embedding=args.dim).cuda(current_device)
    ...
    for epoch in range(args.max_epochs):
        ...
        if test_loss.avg < best_loss and rank == 0:
            ...
            if not args.fold:
                args.fold = 0
            torch.save(model.state_dict(), str(args.exp_name) + f"/best_{args.fold}.pt")
