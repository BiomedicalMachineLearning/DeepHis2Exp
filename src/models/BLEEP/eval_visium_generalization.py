...
if SAVE_EMBEDDINGS:
    for fold in range(1):
        save_embeddings(model_path=f"clip_visium_bc_holdout_alex/best_{fold}.pt", save_path=f"clip_visium_bc_holdout_alex/embeddings_{fold}/",
                        datasize=datasize, dim=5689, fold=fold)
...
spot_expressions = [np.load(f"data/filtered_expression_matrices/{samp}/hvg_matrix_plusmarkers.npy")
                   for samp in samps]
...
for fold  in range(9):
    ...
    save_path = f"clip_visium_bc_holdout_alex/embeddings_{0}/"
    ...
    save_path=f"clip_visium_bc_holdout_alex/preds_{fold}/"
    import os
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if save_path != "":
    ...
    expression_gt = np.load(f"data/filtered_expression_matrices/{samps[fold]}/hvg_matrix_plusmarkers.npy")
    matched_spot_expression_pred_1 = np.load(f"clip_visium_bc_holdout_alex/preds_{fold}/matched_spot_expression_pred.npy")
    ...
    df.to_csv(f"../../results/ffpe_new_holdout_alex/bleep_cor_{samps[fold]}.csv")
