...
if SAVE_EMBEDDINGS:
    for fold in range(9):
        save_embeddings(model_path=f"clip_visium_bc_6subset/best_{fold}.pt", save_path=f"clip_visium_bc_6subset/embeddings_{fold}/",
                        datasize=datasize, dim=685, fold=fold)
...
spot_expressions = [np.load(f"data/filtered_expression_matrices/{samp}/hvg_matrix_plus6_subset.npy")
                   for samp in samps]
...
for fold  in range(9):
    ...
    save_path = f"clip_visium_bc_6subset/embeddings_{fold}/"
    ...
    save_path=f"clip_visium_bc/preds_{fold}/"
    import os
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if save_path != "":
    ...
    expression_gt = np.load(f"data/filtered_expression_matrices/{samps[fold]}/hvg_matrix_plus6_subset.npy")
    matched_spot_expression_pred_1 = np.load(f"clip_visium_bc/preds_{fold}/matched_spot_expression_pred.npy")
    ...
    with open("hvgs_union.pickle6", 'rb') as a:
        hvg_union = pickle.load(a)

    # gene_list = ["COX6C","TTLL12", "HSP90AB1", 
    #        "TFF3", "ATP1A1", "B2M", "FASN", "SPARC", "CD74", "CD63", "CD24", "CD81"]
    with open('/afm03/Q2/Q2051/STimage_project/STimage_dataset/gene_list/1000hvg_common.pkl', 'rb') as f:
    ...
    df.to_csv(f"../../results/ffpe_6/bleep_cor_{samps[fold]}_subset.csv")
