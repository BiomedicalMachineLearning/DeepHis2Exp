...
def plot_correlation_scatter_hist(outDir, valid_labels, valid_preds, geneSymbols, scatter, name):
    ...
    corR_df = pd.DataFrame(columns=['geneSymbols','corR'] )
    ...
    for i in range(len(geneSymbols)):
        ...
        corR_spearman, _ = spearmanr(lab, pred)


# Experimental fix https://stackoverflow.com/questions/31323499/sklearn-error-valueerror-input-contains-nan-infinity-or-a-value-too-large-for
#         lab = lab.reset_index()
...
#         rmse_minmax = np.sqrt(mean_squared_error(lab, minmax_scale(pred)))
        ...
        # corR_df = corR_df.append(pd.Series([geneSymbols[i],corR_tmp,corR_spearman,rmse,rmse_minmax], index=['geneSymbols','corR','corS','RMSE','RMSE_MinMax']), ignore_index=True)
        corR_df = corR_df.append(pd.Series([geneSymbols[i],corR_tmp,corR_spearman], index=['geneSymbols','corR','corS']), ignore_index=True)
