...
os.environ["CV_IO_MAX_IMAGE_WIDTH"] = pow(2,30).__str__()
os.environ["CV_IO_MAX_IMAGE_HEIGHT "] = pow(2,30).__str__()
...
I = Image.open(dirName+"/image.tif")
...
with open(dirName+"/spatial/tissue_positions_list.csv") as f:
    first_line = f.readline()
if 'in_tissue' in first_line:
    tissue_pos = pd.read_csv(dirName+"/spatial/tissue_positions_list.csv")
else:
    tissue_pos = pd.read_csv(dirName+"/spatial/tissue_positions_list.csv", header=None)
...
### load cluster list ###
# cluster_list = pd.read_csv(dirName+"/analysis/clustering/graphclust/clusters.csv")
...
for i in cluster_pos_df.index:
    print(i)
    print(cluster_pos_df.loc[i,'image_path'])
