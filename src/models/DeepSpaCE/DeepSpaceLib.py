...
def makeDataList(rootDir, sampleNames, clusteringMethod, extraSize, geneSymbols, quantileRGB, seed, cross_index, train_equals_test, is_test, rm_cluster):
    ...
    #     tmp = pd.read_csv(rootDir+"/"+sample+"/analysis/clustering/"+clusteringMethod+"/clusters.csv")
    #     tmp['Sample'] = sample
    ...
    for sample in sampleNames:
        with open(rootDir+"/"+sample+"/spatial/tissue_positions_list.csv") as f:
            first_line = f.readline()
        if 'in_tissue' in first_line:
            print("AAAAAAAAAAAAAA")
            tmp = pd.read_csv(rootDir+"/"+sample+"/spatial/tissue_positions_list.csv")
        else:
            tmp = pd.read_csv(rootDir+"/"+sample+"/spatial/tissue_positions_list.csv", header=None)
    ...
    for sample in sampleNames:
        ...
        tmp = tmp.reindex(columns=geneSymbols)
...
#        exp_mat.iloc[:,i] = minmax_scale(exp_mat.iloc[:,i].tolist())
    ...
    # import pickle
    # with open('data_list_df.pkl', 'wb') as file:
    #     pickle.dump(data_list_df, file)
