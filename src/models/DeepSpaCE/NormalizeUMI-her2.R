...
# load Visium data
# visiumData <- Load10X_Spatial(paste0(dataDir,"/",sampleName,"/"),
...
ST.mtx <- function(path # The Spaceranger output directory which includes filtered_feature_bc_matrix and spatial folders
  ...
  x1 <- Read10X(path,gene.column = 1) # Reading in the gene count matrix
  x2 <- Read10X_Image(path, image.name='image.png') # Reading in the ST image
  ...
  x1@images$x1 <- Read10X_Image(path, image.name='image.png',filter.matrix = T) # Adding the image to the Seurat object
...
# https://github.com/satijalab/sctransform/issues/131#issuecomment-1494394748
visiumData <- ST.mtx(paste0(dataDir,"/",sampleName,"/"),sampleName)

visiumData=visiumData[,unname(which(colSums(GetAssayData(visiumData))!=0))]

# # https://stackoverflow.com/questions/73131436/error-in-funleft-right-non-numeric-argument-to-binary-operator-when-runni
# visiumData@images[["slice1"]]@coordinates[["tissue"]] <- as.integer(visiumData@images[["slice1"]]@coordinates[["tissue"]])
# visiumData@images[["slice1"]]@coordinates[["row"]] <- as.integer(visiumData@images[["slice1"]]@coordinates[["row"]])
# visiumData@images[["slice1"]]@coordinates[["col"]] <- as.integer(visiumData@images[["slice1"]]@coordinates[["col"]])
# visiumData@images[["slice1"]]@coordinates[["imagerow"]] <- as.integer(visiumData@images[["slice1"]]@coordinates[["imagerow"]])
# visiumData@images[["slice1"]]@coordinates[["imagecol"]] <- as.integer(visiumData@images[["slice1"]]@coordinates[["imagecol"]])
...
# SCTransform
visiumData_SCT <- SCTransform(visiumData, assay = "Spatial", verbose = FALSE, min_cells=3) ######### LESS FILTERING https://github.com/satijalab/sctransform/issues/100 (1 will throw compute error https://github.com/satijalab/seurat/issues/4296)
...
print("Colnames___________")
print(colnames(mat))
print(colnames(mat_SCT))

##### FOR SOME REASON, X gets appended to the colnames (barcodes)
##### REMOVE IT HERE
colnames(mat) <- sub('.', '', colnames(mat))
colnames(mat_SCT) <- sub('.', '', colnames(mat_SCT))
...
# SCTransform
visiumData_fil_SCT <- SCTransform(visiumData_fil, assay = "Spatial", verbose = FALSE, min_cells=3) ######### LESS FILTERING https://github.com/satijalab/sctransform/issues/100 (1 will throw compute error https://github.com/satijalab/seurat/issues/4296)
...
### ERRORS BELOW BUT PROBABLY NOT ESSENTIAL


## define paths
sample_names <- c(sampleName)

image_paths <- paste0(dataDir,"/",sample_names,"/spatial/tissue_lowres_image.png")

scalefactor_paths <- paste0(dataDir,"/",sample_names,"/spatial/scalefactors_json.json")

tissue_paths <- paste0(dataDir,"/",sample_names,"/spatial/tissue_positions_list.csv")

# cluster_paths <- paste0(dataDir,"/",sample_names,"/analysis/clustering/graphclust/clusters.csv")

matrix_paths <- paste0(dataDir,"/",sample_names,"/filtered_feature_bc_matrix/")
...
for (i in 1:length(sample_names)) {
  ...
  con <- file(tissue_paths[i],"r")
  first_line <- readLines(con,n=1)
  close(con)
  if (grepl("barcode", first_line, fixed = TRUE)) {
    bcs[[i]] <- read.csv(tissue_paths[i], header = TRUE)
    colnames(bcs[[i]])=c("barcode","tissue","row","col","imagerow","imagecol")
