...
adata_all.obs['tile_path'] = adata_all.obs['tile_path'].apply(lambda x: x.replace('/clusterdata/uqxtan9/Xiao/breast_cancer_9visium/','../data/dataset_breast_cancer_9visium/'))
...
i = int(sys.argv[1])
test_sample = ['1160920F', 'CID4290', 'block1','block2','FFPE']
...
# original
# train_gen_ = train_gen.shuffle(buffer_size=500).batch(64).repeat(1).cache().prefetch(tf.data.experimental.AUTOTUNE)
...
train_gen_ = train_gen.shuffle(buffer_size=500).batch(128).repeat(1).cache().prefetch(tf.data.experimental.AUTOTUNE)
...
valid_gen_ = valid_gen.shuffle(buffer_size=500).batch(128).repeat(1).cache().prefetch(tf.data.experimental.AUTOTUNE)
...
print(f"Finished training in {end_train-start_train}")
...
for test_sample in test_sample:
    ...
    # with open(f"../results/ffpe_new_holdout/stnet_preds_{test_sample}.pkl", 'wb') as f:
    ...
    df.to_csv("../results/visium_generalization/stnet_cor_{}.csv".format(test_sample))
    
    
    with open("../results/visium_generalization/stnet_times.txt", 'a') as f:
        f.write(f"{test_sample} {end_train - start_train} - {time.strftime('%H:%M:%S', time.localtime())}")

df.to_csv("../results/visium_generalization/stnet_cor_all.csv")
