...
class ViT_Anndata(torch.utils.data.Dataset):
    ...
    def __init__(self, adata_dict, train_set, gene_list, processed_img=None, train=True,r=4,norm="lognorm",flatten=False, ori=True,prune='NA',neighs=4):
...
#         if patches is None:
#             patches = torch.tensor(self.processed_img[ID]).permute(0,3,1,2).to(torch.float32)
#             self.patch_dict[ID]=patches
        data=[patches, positions, exps, adj]
        if self.ori:
