...
class ViT_Anndata(torch.utils.data.Dataset):
    """ Visium breast cancer datasets """
    def __init__(self, adata_dict, train_set, gene_list, train=True,r=4,norm=False,flatten=False,ori=True,prune='NA',neighs=4):
...
#        self.img_dict = {i:torch.Tensor(np.array(self.get_img(i))) for i in self.names}
        ...
        self.label = {i:torch.tensor(m.obs['in_cancer'].values) for i,m in self.adata_dict.items()} # 0:normal cell type, 1:cancer cell type, 6: missing label
    ...
    def __getitem__(self, index):
        ...
        data=[patches, positions, exps, adj, label]
        if self.ori:
