...
if dataset_name == "Skin_Melanoma":
    ...
    if model_name in ["stnet", "deeppt", "stimage", "bleep", "deepspace"]:
        full_train_dataset = CSCC(train=True,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="log1p", num_subsets=12, gene_list=target_gene_list)
        test_dataset = CSCC(train=False,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="log1p", num_subsets=12, gene_list=target_gene_list)
    else:
        full_train_dataset = CSCC(train=True,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="lognorm", num_subsets=3, gene_list=target_gene_list)
        test_dataset = CSCC(train=False,fold=fold,r=112, neighs=8, num_subsets=3, color_norm=colornorm, exp_norm="lognorm", gene_list=target_gene_list)

elif dataset_name == "BC_Her2ST":
    if model_name in ["stnet", "deeppt", "stimage", "bleep", "deepspace"]:
        full_train_dataset = Her2st(train=True,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="log1p", gene_list=target_gene_list)
        test_dataset = Her2st(train=False,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="log1p", gene_list=target_gene_list)
    else:
        full_train_dataset = Her2st(train=True,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="lognorm", gene_list=target_gene_list)
        test_dataset = Her2st(train=False,fold=fold,r=112, neighs=8, color_norm=colornorm, exp_norm="lognorm", gene_list=target_gene_list)
