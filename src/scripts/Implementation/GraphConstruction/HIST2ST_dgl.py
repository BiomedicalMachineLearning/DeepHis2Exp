...
class mixer_transformer(nn.Module):
    def __init__(self,channel=32, kernel_size=5, dim=1024,
        ...
        self.layer3=GNN_dgl(num_feature=dim, nhid=dim, gnn="GCN-mean")
    ...
    def forward(self,x,ct,adj, edge_index, edge_weight):
        x=self.down(self.layer1(x)).unsqueeze(0)
        x=self.layer2(x+ct).squeeze(0)
        # Hist2st default version
        # jk=[]
        ...
        # Hist2st dgl weighted graph version
        u, v = edge_index[0].squeeze(0), edge_index[1].squeeze(0)
        g = dgl.graph((u, v))
        g.edata['w'] = edge_weight.squeeze(0)
        x=self.layer3(g, x, g.edata['w'])
        return x
...
class ViT(nn.Module):
    ...
    def forward(self,x,ct,adj, edge_index, edge_weight):
        x = self.dropout(x)
        x = self.transformer(x,ct,adj, edge_index, edge_weight)
        return x

class Hist2ST_dgl(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, fig_size=112, label=None, 
                 dropout=0.2, n_pos=128, kernel_size=5, patch_size=7, n_genes=785, 
                 depth1=2, depth2=8, depth3=4, heads=16, channel=32, dim=1024,
    ...
    def forward(self, patches, centers, adj, edge_index, edge_weight, aug=False):
        ...
        h = self.vit(patches,ct,adj, edge_index, edge_weight)
        x = self.gene_head(h)
        extra=None
        if self.zinb>0:
            if self.nb:
    ...
    def aug(self,patch, center, adj, edge_index, edge_weight):
        bake_x=[]
        for i in range(self.bake):
            new_patch=self.tf(patch.squeeze(0)).unsqueeze(0)
            x,_,h=self(new_patch,center,adj, edge_index, edge_weight, True)
    ...
    def training_step(self, batch, batch_idx):
        patch, center, exp, adj, oris, sfs, edge_index, edge_weight = batch
        adj, exp=adj.squeeze(0), exp.squeeze(0)
        pred,extra,h = self(patch, center, adj, edge_index, edge_weight)
        mse_loss = F.mse_loss(pred, exp)
        self.log('mse_loss', mse_loss,on_epoch=True, prog_bar=True, logger=True)
        bake_loss=0
        if self.bake>0:
            bake_x=self.aug(patch, center, adj, edge_index, edge_weight)
